...
namespace RealtimeMesh
{
	...
	struct TRealtimeMeshStreamDataAccessor<InAccessType, void, bAllowSubstreamAccess>
	{
		...
		// The converters are stored by value (a snapshot copied out under the registry read lock in
		// InitializeContext) rather than as references into the global map. A reference would dangle
		// if a concurrent Register/Unregister rehashed the map while this context is still live.
		struct TContextWithoutSubstream
		{
			typename TCopyQualifiersFromTo<InAccessType, FRealtimeMeshStream>::Type& Stream;
			FRealtimeMeshElementConverters ReadConverters;
			FRealtimeMeshElementConverters WriteConverters;
		};
		struct TContextWithSubstream
		{
			typename TCopyQualifiersFromTo<InAccessType, FRealtimeMeshStream>::Type& Stream;
		...
		{			
			...
			// Snapshot the read/write converters by value while the registry read lock is held, so the
			// context owns copies rather than references that could dangle on a concurrent map rehash.
			TOptional<FRealtimeMeshElementConverters> ReadConverters;
			TOptional<FRealtimeMeshElementConverters> WriteConverters;
			FRealtimeMeshTypeConversionUtilities::ProcessTypeConverter(StreamElementTypeDef, AccessElementTypeDef,
				[&](const FRealtimeMeshElementConverters& Converters) { ReadConverters.Emplace(Converters); });
			FRealtimeMeshTypeConversionUtilities::ProcessTypeConverter(AccessElementTypeDef, StreamElementTypeDef,
				[&](const FRealtimeMeshElementConverters& Converters) { WriteConverters.Emplace(Converters); });
			...
			if constexpr (bAllowSubstreamAccess)
			{
				...
				{
					Stream,
					ReadConverters.GetValue(),
					WriteConverters.GetValue(),
		...
		// Whole-span contiguous write fast path. When the access covers complete, tightly-packed rows
		// (no substream offset, tightly-packed source rows, and a stream stride equal to the element stride
		// times the element count) the source span already matches the destination layout, so a single
		// contiguous conversion of every element replaces one indirect converter call per row. Returns
		// false when the layout doesn't allow it, leaving the caller to fall back to the per-row path.
		static bool SetBufferRange(const TContext& Context, int32 StartIndex, const AccessType* Elements, int32 Count)
		{
			constexpr uint32 NumElements = AccessTypeTraits::NumElements;

			// The source AccessType array must be tightly packed for a contiguous element walk to be valid.
			if constexpr (sizeof(AccessType) != NumElements * sizeof(AccessElementType))
			{
				return false;
			}
			else
			{
				if constexpr (bAllowSubstreamAccess)
				{
					if (Context.ElementOffset != 0)
					{
						return false;
					}
				}

				// Destination rows must be tightly packed (no inter-row padding, exactly NumElements each).
				if (Context.Stream.GetStride() != Context.Stream.GetElementStride() * static_cast<int32>(NumElements))
				{
					return false;
				}

				Context.WriteConverters.ConvertContiguousArray(Elements, Context.Stream.GetDataRawAtVertex(StartIndex), NumElements * static_cast<uint32>(Count));
				return true;
			}
		}
		static AccessElementType GetElementValue(const TContext& Context, int32 Index, int32 ElementIndex)
...
#undef DEFINE_BINARY_OPERATOR_VARIATIONS	
	...
	public:
		...
		FORCEINLINE TEnableIfWritable<void, U> SetRange(int32 StartIndex, TArrayView<const AccessType> Elements)
		{
			...
			// Identity fast path: when the access type is bit-identical to the buffer
			// type (no conversion) and each row is exactly one AccessType (stride check
			// rules out a wider stream / substream offset), the source view is already
			// in the destination's memory layout, so bulk-copy it in one memcpy instead
			// of a per-row conversion loop.
			if constexpr (std::is_same_v<InAccessType, InBufferType>)
			{
				if (Context.Stream.GetStride() == sizeof(AccessType))
				{
					FMemory::Memcpy(Context.Stream.GetDataRawAtVertex(StartIndex), Elements.GetData(), sizeof(AccessType) * static_cast<size_t>(Elements.Num()));
					return;
				}
			}
			else if constexpr (std::is_same_v<InBufferType, void>)
			{
				// Runtime-typed (void-buffer) builder: when the access covers whole, tightly-packed rows,
				// convert the entire span in a single contiguous conversion instead of a per-row converter call.
				if (StreamDataAccessor::SetBufferRange(Context, StartIndex, Elements.GetData(), Elements.Num()))
				{
					return;
				}
			}
	...
	{
		// This template is compile-time-typed only. For runtime-determined layout
		// (the format isn't known until enable-time), use FRealtimeMeshDynamicBuilder.
		static_assert(!std::is_void_v<IndexType>, "TRealtimeMeshBuilderLocal requires a concrete IndexType. For runtime layout, use FRealtimeMeshDynamicBuilder.");
		static_assert(!std::is_void_v<TangentElementType>, "TRealtimeMeshBuilderLocal requires a concrete TangentElementType. For runtime layout, use FRealtimeMeshDynamicBuilder.");
		static_assert(!std::is_void_v<TexCoordElementType>, "TRealtimeMeshBuilderLocal requires a concrete TexCoordElementType. For runtime layout, use FRealtimeMeshDynamicBuilder.");
		static_assert(!std::is_void_v<PolyGroupIndexType>, "TRealtimeMeshBuilderLocal requires a concrete PolyGroupIndexType. For runtime layout, use FRealtimeMeshDynamicBuilder.");
		...
		using TriangleStreamType = TIndex3<IndexType>;
		using TangentStreamType = TRealtimeMeshTangents<TangentElementType>;
		using TexCoordStreamType = TRealtimeMeshTexCoords<TexCoordElementType, NumTexCoords>;
		...
		static_assert(FRealtimeMeshBufferTypeTraits<TexCoordStreamType>::NumElements == NumTexCoords, "NumTexCoords must match the number of elements in the TexCoordStreamType");
	...
	private:
		...
		// Per-pool stream linkages owned by this builder. They keep the streams in
		// each pool size-locked while this builder exists; on builder destruction
		// the linkages' destructors unlink every member stream. (Linkage is
		// non-copyable/non-movable, which transitively makes this builder
		// non-copyable/non-movable — fine, since all existing callsites use
		// stack-scoped or TUniquePtr-held builders.)
	...
	public:
		...
		TRealtimeMeshBuilderLocal(FRealtimeMeshStreamSet& InExistingStreams)
			...
			, Triangles(GetStreamBuilder<TriangleAccessType, TriangleStreamType>(FRealtimeMeshStreams::Triangles, true,
				GetRealtimeMeshBufferLayout<TriangleStreamType>())->GetStream())
		...
		{
			VerticesLinkage.BindStream(Vertices.GetStream(), FRealtimeMeshStreamDefaultRowValue());
			if (Tangents.IsSet())
			{
				VerticesLinkage.BindStream(Tangents->GetStream(),
			...
			if (TexCoords.IsSet())
			{
				VerticesLinkage.BindStream(TexCoords->GetStream(), FRealtimeMeshStreamDefaultRowValue());
			}

			if (Colors.IsSet())
			{
				VerticesLinkage.BindStream(Colors->GetStream(), FRealtimeMeshStreamDefaultRowValue::Create(FColor::White));
			}

			TrianglesLinkage.BindStream(Triangles.GetStream(), FRealtimeMeshStreamDefaultRowValue());

			if (TrianglePolyGroups.IsSet())
			{
				TrianglesLinkage.BindStream(TrianglePolyGroups->GetStream(), FRealtimeMeshStreamDefaultRowValue());
			}

			if (DepthOnlyTriangles.IsSet())
			{
				DepthOnlyTrianglesLinkage.BindStream(DepthOnlyTriangles->GetStream(), FRealtimeMeshStreamDefaultRowValue());
			}
			if (DepthOnlyTrianglePolyGroups.IsSet())
			{
				DepthOnlyTrianglesLinkage.BindStream(DepthOnlyTrianglePolyGroups->GetStream(), FRealtimeMeshStreamDefaultRowValue());
			}
		...
		void EnableTangents(const FRealtimeMeshElementType& ElementType)
		{
			...
			checkf(ElementType == GetRealtimeMeshDataElementType<TangentElementType>(),
				TEXT("ElementType must match this builder's TangentElementType. For runtime-determined tangent type, use FRealtimeMeshDynamicBuilder."));
		...
		template<typename NewTangentElementType = TangentElementType>
		void EnableTangents()
		{
			static_assert(std::is_same_v<TangentElementType, NewTangentElementType>,
				"NewTangentElementType must match the builder's TangentElementType. For runtime-determined tangent type, use FRealtimeMeshDynamicBuilder.");
		...
		void EnableTexCoords(const FRealtimeMeshElementType& ElementType, int32 NumChannels = 1)
		{
			...
			checkf(ElementType == GetRealtimeMeshDataElementType<TexCoordElementType>(),
				TEXT("ElementType must match this builder's TexCoordElementType. For runtime-determined tex coord type, use FRealtimeMeshDynamicBuilder."));
		...
		template<typename NewTexCoordElementType = TexCoordElementType>
		void EnableTexCoords(int32 NumChannels = NumTexCoords)
		{
			static_assert(std::is_same_v<TexCoordElementType, NewTexCoordElementType>,
				"NewTexCoordElementType must match the builder's TexCoordElementType. For runtime-determined tex coord type, use FRealtimeMeshDynamicBuilder.");
		...
		void EnableDepthOnlyTriangles(const FRealtimeMeshElementType& ElementType)
		{
			...
			checkf(ElementType == GetRealtimeMeshDataElementType<IndexType>(),
				TEXT("ElementType must match this builder's IndexType. For runtime-determined index type, use FRealtimeMeshDynamicBuilder."));
		...
		template<typename NewIndexType = IndexType>
		void EnableDepthOnlyTriangles()
		{
			static_assert(std::is_same_v<IndexType, NewIndexType>,
				"NewIndexType must match the builder's IndexType. For runtime-determined index type, use FRealtimeMeshDynamicBuilder.");
		...
		void EnablePolyGroups(const FRealtimeMeshElementType& ElementType)
		{
			...
			checkf(ElementType == GetRealtimeMeshDataElementType<PolyGroupIndexType>(),
				TEXT("ElementType must match this builder's PolyGroupIndexType. For runtime-determined poly group index type, use FRealtimeMeshDynamicBuilder."));
		...
		template<typename NewPolyGroupIndexType = PolyGroupIndexType>
		void EnablePolyGroups()
		{
			static_assert(std::is_same_v<PolyGroupIndexType, NewPolyGroupIndexType>,
				"NewPolyGroupIndexType must match the builder's PolyGroupIndexType. For runtime-determined poly group index type, use FRealtimeMeshDynamicBuilder.");
		...
		{
			// Route through the linkage batch path: adds one row to every stream
			// bound to VerticesLinkage in a single pass (Position zeroed via its
			// empty default, the rest filled with their bound default row value),
			// instead of paying a per-stream broadcast from Vertices.AddZeroed().
			const SizeType Index = VerticesLinkage.AddRowsZeroed(1);
		...
		void SetColor(int32 VertIdx, FColor VertexColor)
		{
			...
			// Setters address an existing row; auto-growing here would silently extend
			// every linked vertex stream. Guard the index like SetPosition/SetNormal.
			checkf(VertIdx < Colors->Num(), TEXT("SetColor index %d out of bounds (Num %d); append the vertex first"), VertIdx, Colors->Num());
		...
		{
			// Batch add one row across TrianglesLinkage (Triangles + PolyGroups
			// when bound) in a single pass instead of a per-stream broadcast.
			const SizeType Result = TrianglesLinkage.AddRowsZeroed(1);
			Triangles.Set(Result, Triangle);
			return Result;
		}
		...
		{
			const SizeType Result = TrianglesLinkage.AddRowsZeroed(1);
			Triangles.Set(Result, TIndex3<uint32>(Vert0, Vert1, Vert2));
			return Result;
		}
		...
		{
			checkf(HasDepthOnlyTriangles(), TEXT("Depth only triangles not enabled"));
			// Batch add one row across DepthOnlyTrianglesLinkage in one pass.
			const int32 Result = DepthOnlyTrianglesLinkage.AddRowsZeroed(1);
			DepthOnlyTriangles->Set(Result, Triangle);
			return Result;
		}
		...
		int32 AddDepthOnlyTriangle(uint32 Vert0, uint32 Vert1, uint32 Vert2)
		{
			checkf(HasDepthOnlyTriangles(), TEXT("Depth only triangles not enabled"));
			const int32 Result = DepthOnlyTrianglesLinkage.AddRowsZeroed(1);
			DepthOnlyTriangles->Set(Result, TIndex3<uint32>(Vert0, Vert1, Vert2));
			return Result;
		}
