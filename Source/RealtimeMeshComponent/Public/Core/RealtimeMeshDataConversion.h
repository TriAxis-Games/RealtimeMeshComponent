...
namespace RealtimeMesh
{
	...
	public:
		...
		// Looks up the converter for the given type pair and invokes Func with it while holding the
		// registry read lock, so the reference passed to Func can never outlive the lock (a concurrent
		// Register/Unregister rehash can't invalidate it mid-use). Do NOT stash the reference for later.
		static void ProcessTypeConverter(const FRealtimeMeshElementType& FromType, const FRealtimeMeshElementType& ToType, TFunctionRef<void(const FRealtimeMeshElementConverters&)> Func);
