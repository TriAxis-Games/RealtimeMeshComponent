...
{	
	...
	namespace Internal
	{
		...
		{
			Normal.W = static_cast<int16>(FMath::Clamp<int32>(FMath::RoundToInt(Sign * MAX_int16), MIN_int16, MAX_int16));
		}
		...
		// Drop-in replacement for GetBasisDeterminantSign(Tangent, Binormal, Normal) that computes the
		// tangent-basis determinant sign directly instead of constructing a full FMatrix and calling
		// RotDeterminant. It reproduces RotDeterminant's exact double-precision evaluation order for the
		// basis matrix { row0 = Tangent, row1 = Binormal, row2 = Normal }, so the resulting sign — and
		// therefore the packed tangent W bit — is bit-identical to GetBasisDeterminantSign for every input,
		// including degenerate/zero frames (a zero determinant yields +1.0f, matching the < 0 test).
		inline float RealtimeMeshBasisDeterminantSign(const FVector& Tangent, const FVector& Binormal, const FVector& Normal)
		{
			const double Det =
				Tangent.X * (Binormal.Y * Normal.Z - Binormal.Z * Normal.Y) -
				Binormal.X * (Tangent.Y * Normal.Z - Tangent.Z * Normal.Y) +
				Normal.X * (Tangent.Y * Binormal.Z - Tangent.Z * Binormal.Y);
			return (Det < 0) ? -1.0f : +1.0f;
		}
	...
	public:
		...
		TRealtimeMeshTangents(const FVector3f& InNormal, const FVector3f& InBinormal, const FVector3f& InTangent)
			: Tangent(InTangent), Normal(FVector4f(InNormal, Internal::RealtimeMeshBasisDeterminantSign(FVector3d(InTangent), FVector3d(InBinormal), FVector3d(InNormal))))
		{
		}
		...
		TRealtimeMeshTangents(const FVector3d& InNormal, const FVector3d& InBinormal, const FVector3d& InTangent)
			...
			, Normal(FVector4f(FVector3f(InNormal), Internal::RealtimeMeshBasisDeterminantSign(InTangent, InBinormal, InNormal)))
		{
		}
		...
		// Decode through GetTangentAsVector so this compiles for every tangent representation
		// (FPackedNormal has no direct .W member) — matches GetBinormal()'s TangentZ.W usage.
		bool IsBinormalFlipped() const { return Internal::GetTangentAsVector(Normal).W < 0.0f; }
		...
		void SetNormal(const FVector3f& NewNormal)
		{
			...
			const FVector4f NewTangentZ(NewNormal, Internal::RealtimeMeshBasisDeterminantSign(FVector3d(TangentX), FVector3d(TangentY), FVector3d(NewNormal)));
			Normal = NewTangentZ;
		}
		...
		void SetTangent(const FVector3f& NewTangent)
		{
			...
			TangentZ.W = Internal::RealtimeMeshBasisDeterminantSign(FVector3d(NewTangent), FVector3d(TangentY), FVector3d(TangentZ));
			Normal = TangentZ;
			Tangent = NewTangent;
		}
		...
		{
			Normal = FVector4f(InNormal, Internal::RealtimeMeshBasisDeterminantSign(FVector3d(InTangent), FVector3d(InBinormal), FVector3d(InNormal)));
			Tangent = InTangent;
		}
	...
	{
		// Hash the raw bytes of the index triple.
		// Note: this assumes there's no padding that could contain uncompared data.
		return FCrc::MemCrc32(&Index, sizeof(TIndex3<IndexType>));
	}
	...
	// Intentionally an unscoped enum, unlike the scoped enums used elsewhere. The IET_*
	// constants are public API used widely by callers; scoping it would break that surface.
	...
	public:
		...
		REALTIMEMESHCOMPONENT_API static const FRealtimeMeshElementType Invalid;
	};
	...
	public:
		...
		REALTIMEMESHCOMPONENT_API static const FRealtimeMeshBufferLayout Invalid;
	};
	...
	{
	private:
		// Row stride = ElementStride * NumElements. Must be wider than uint8 because
		// large layouts (e.g. FVector4d x 8 = 256 bytes) overflow a uint8 to 0.
		const uint16 Stride;
	...
	public:
		...
		FRealtimeMeshBufferMemoryLayout(uint8 InElementStride, uint8 InAlignment, uint8 InNumElements)
			: Stride(static_cast<uint16>(InElementStride) * InNumElements), ElementStride(InElementStride), Alignment(InAlignment) { }

		FORCEINLINE bool IsValid() const { return Stride > 0; }

		FORCEINLINE uint16 GetStride() const { return Stride; }
