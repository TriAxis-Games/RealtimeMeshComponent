...
namespace RealtimeMesh
{
	...
	const TMap<FRealtimeMeshElementType, FRealtimeMeshElementTypeDetails> FRealtimeMeshBufferLayoutUtilities::SupportedTypeDefinitions =
	{
		...
		// NOTE (DUP-016): The normalized color mapping (VET_Color / PF_R8G8B8A8) is the effective
		// UInt8x4 entry — TMap brace-init keeps the last write on a duplicate key. Two raw-integer
		// variants for this same key (VET_UByte4 and VET_UByte4N, both PF_R8G8B8A8_UINT) were
		// declared earlier and were shadowed dead entries; they have been removed. This survivor is
		// pinned by the TypeRegistry characterization test.
		{
